...
export const shiftPresensiSchema = z.object({
  ...
  kategori_shift: z.enum(kategori, `Kategori Shift wajib ${kategori.join('/')}`),
  waktu_mulai: z.string().nonempty('Waktu Mulai wajib diisi'),
  waktu_selesai: z.string().nonempty('Waktu Selesai wajib diisi'),
  toleransi_menit: z.number().min(0, 'Toleransi Menit wajib diisi').max(100, 'Toleransi Menit wajib diisi'),
