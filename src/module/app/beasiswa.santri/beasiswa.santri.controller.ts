...
export default class Controller {
  public async list(req: Request, res: Response) {
    try {
      ...
      return helper.catchError(`beasiswa santri list: ${err?.message}`, 500, res);
    }
  }
  ...
  public async index(req: Request, res: Response) {
    try {
      ...
      return helper.catchError(`beasiswa santri index: ${err?.message}`, 500, res);
    }
  }
  ...
  public async detail(req: Request, res: Response) {
    try {
      ...
      const result: Object | any = await repository.detail({ id_beasiswasantri: id });
      if (!result) return response.success(NOT_FOUND, null, res, false);
      return response.success(SUCCESS_RETRIEVED, result, res);
    } catch (err: any) {
      return helper.catchError(`beasiswa santri detail: ${err?.message}`, 500, res);
    }
  }
