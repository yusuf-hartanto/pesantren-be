...
export const up = async (queryInterface: QueryInterface) => {
  return queryInterface.sequelize.transaction(async (transaction) => {
    ...
    await queryInterface.changeColumn('orang_tua_wali', 'pekerjaan', {
      type: DataTypes.ENUM(
        'Tidak Bekerja', 'Ibu Rumah Tangga', 'Petani', 'Buruh Harian',
        'Nelayan', 'Wiraswasta', 'Pedagang', 'Karyawan Swasta', 'PNS',
        'TNI / POLRI', 'Guru / Dosen', 'Pekerja Migran', 'Pensiunan', 'Lainnya'
      ),
      allowNull: true,
    }, { transaction });
    ...
    for (const [colName, config] of Object.entries(newColumns)) {
      if (!tableDesc[colName]) {
        await queryInterface.addColumn('orang_tua_wali', colName, config as any, { transaction });
      }
    }
  });
};
...
export const down = async (queryInterface: QueryInterface) => {
  return queryInterface.sequelize.transaction(async (transaction) => {
    ...
    const columnsToRemove = [
      'keterangan', 'deleted_at', 'is_deleted', 'penghasilan',
      'province_id', 'city_id', 'district_id', 'sub_district_id'
    ];

    for (const col of columnsToRemove) {
      if (tableDesc[col]) {
        await queryInterface.removeColumn('orang_tua_wali', col, { transaction });
      }
    }

    // Kembalikan ke STRING
    await queryInterface.changeColumn('orang_tua_wali', 'pendidikan', { type: DataTypes.STRING(255), allowNull: true }, { transaction });
    await queryInterface.changeColumn('orang_tua_wali', 'pekerjaan', { type: DataTypes.STRING(255), allowNull: true }, { transaction });
    await queryInterface.changeColumn('orang_tua_wali', 'hubungan', { type: DataTypes.STRING(255), allowNull: true }, { transaction });

    // Bersihkan ENUM types
    const enums = ['hubungan', 'pendidikan', 'pekerjaan', 'penghasilan'];
    for (const e of enums) {
      await queryInterface.sequelize.query(`DROP TYPE IF EXISTS "enum_orang_tua_wali_${e}";`, { transaction });
    }
  });
};